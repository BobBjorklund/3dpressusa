...
export default function FourColorDesignStudio() {
  ...
  async function downloadLayeredSvgZip(): Promise<void> {
    ...
    for (const color of usedColors) {
      ...
      // Export uses smooth vector paths — svg_stack_to_placard_3mf.py traces
      // <path> boundary rings (even-odd union) instead of a rasterised grid.
      const pathData = buildSmoothPaths(data, width, height, rgb, TILE_SIZE_MM / width);
      if (pathData) svgParts.push(`<path d="${pathData}" fill="${color.hex}"/>`);
